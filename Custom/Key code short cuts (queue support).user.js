// ==UserScript==
// @name         Key code short cuts (queue support)
// @version      1.7.9
 ...
 */
 ...
 GM_config.init({
   ...
   queuekey1: {
     label: GM_config.create("Card queue key 1"),
     type: "text",
     default: "q",
   },
   queuekey2: {
     label: GM_config.create("Card queue key 2"),
     type: "text",
     default: "Q",
   },
   skipkey1: {
 ...
 (function () {
   ...
   Mousetrap.bind(
     ...
     function (ev) {
       ...
       var i;
       var stuff;
       stuff = document.getElementsByClassName("cardprice");
   ...
   Mousetrap.bind([GM_config.get("queuekey1"), GM_config.get("queuekey2")], function (ev) {
     let elem = document.querySelector("button.deckcard-lootbox-queue-button");
     if (elem) {
       elem.click();
       let junk_elem = document.querySelector("a.deckcard-junk-button");
       if (junk_elem) {
         junk_elem.classList.remove("deckcard-junk-button");
       }
       elem.classList.remove("deckcard-lootbox-queue-button");
   ...
   Mousetrap.bind([GM_config.get("junkkey1"), GM_config.get("junkkey2")], function (ev) {
     if (document.body.dataset.nname != GM_config.get("MainNation")) {
       ...
       if (elem) {
        ...
        elem.classList.add("disabled");
        let queue_elem = document.querySelector("button.deckcard-lootbox-queue-button");
        queue_elem.classList.remove("deckcard-lootbox-queue-button");
        queue_elem.classList.add("disabled");
       }
     }
   });
